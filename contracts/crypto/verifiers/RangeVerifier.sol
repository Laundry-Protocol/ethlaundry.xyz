...
import "./IRangeVerifier.sol";

/// @title RangeVerifier
/// @notice Groth16 verifier for range proofs (balance >= amount)
/// @dev Verifies that a committed balance is sufficient for a transfer
contract RangeVerifier is IRangeVerifier {
    uint256 constant PRIME_Q = 21888242871839275222246405745257275088696311157297823662689037894645226208583;

    // Verification key (replace with production values from circuit compilation)
    ...
    /// @inheritdoc IRangeVerifier
    function verify(bytes calldata proof, uint256[2] memory publicInputs) external view override returns (bool) {
        if (proof.length != 256) revert InvalidProofLength();

        for (uint256 i = 0; i < 2; i++) {
            if (publicInputs[i] >= PRIME_Q) revert InvalidProof();
        }
    ...
    function _verifyProof(uint256[8] memory proof, uint256[2] memory input) internal view returns (bool) {
    ...
    function _computeVkX(uint256[2] memory input) internal view returns (uint256[2] memory vk_x) {
        vk_x[0] = IC0_X;
        vk_x[1] = IC0_Y;

        uint256[2][2] memory IC = [
            [IC1_X, IC1_Y],
            [IC2_X, IC2_Y]
        ];
